"""
Portfolio backtest using the Kelly-sized strategy (47_kelly_sized_strategy.py)
instead of the flat 100%-when-in version (46_portfolio_backtest.py). Same
22-instrument universe, same equal-weight daily combination, same
asset-matched basket buy-and-hold benchmark.

Run: python 48_kelly_portfolio_backtest.py
Output: kelly_portfolio_backtest_results.json, pnl_plots/_KELLY_PORTFOLIO_backtest.png
"""
import pandas as pd
import numpy as np
import json
...
strat_returns = pd.read_parquet(os.path.join(OUT_DIR, "kelly_strategy_returns.parquet"))
...
portfolio_net = strat_panel.mean(axis=1, skipna=True)
...
with open(os.path.join(OUT_DIR, "kelly_portfolio_backtest_results.json"), "w") as f:
    json.dump(stats, f, indent=2, default=float)

print(f"=== {len(PORTFOLIO_TICKERS)}-instrument equal-weight portfolio, KELLY-sized strategies combined ===")
...
ax.plot(portfolio_cum.index, (portfolio_cum - 1) * 100, color="tab:orange", lw=1.5, label="Kelly-sized strategy portfolio, net")
...
ax.set_title(f"{len(PORTFOLIO_TICKERS)}-instrument portfolio, half-Kelly sizing (cap 2.0x)\n"
             f"Combined strategy {beats} its own basket buy&hold -- alpha vs basket={a_bh:+.1f}%/yr (t={t_bh:+.2f}, "
             f"{'significant' if stats['significant_vs_basket_95'] else 'not significant'})")
ax.set_ylabel("Cumulative return (%)")
ax.legend(fontsize=9, loc="upper left")
fig.tight_layout()
fig.savefig(os.path.join(PLOT_DIR, "_KELLY_PORTFOLIO_backtest.png"), dpi=120, bbox_inches="tight")
plt.close(fig)
print("\nDone.")
