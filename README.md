...
7. **Loss functions and calibrated uncertainty against the same limit** (Paper 15) — tests two further candidates for buying past Paper 14's ceiling. Alternative loss functions (quantile/pinball loss, escalating-order Lq loss) recover no genuine skill or tail behavior. A purpose-built generative downscaler is more informative: its point forecast is, by mathematical construction, provably identical to plain climatology's, but its calibrated predictive *distribution* — matched to each instrument's own predictability limit, corrected for decoder-variance miscalibration, and rescaled for real markets' sub-linear variance growth — beats all five Paper 14 architectures on the Continuous Ranked Probability Score when those architectures are held to the certainty they implicitly claim. Once each architecture instead gets its own uncalibrated uncertainty estimate, the downscaler still beats every sophisticated architecture, but loses outright to climatology's own unmodeled empirical distribution on 7 of 12 instruments — evidence that calibrated uncertainty, not sophistication, is what's being rewarded, and that an unprocessed sample of real recent history is often harder to beat than a carefully calibrated synthetic approximation of the same thing.
8. **Predictability-informed momentum, and the ceiling holds at the decision layer too** (Paper 16) — asks whether decision-layer sophistication (position sizing, instrument selection, signal combination) can buy past the same measured limit rather than better prediction itself. A large-scale reinforcement-learning and genetic-algorithm policy search, predictability-informed conviction weighting, and breadth expansion all fail, extending the wall to a sixth lever. But testing two published momentum strategies the same way this program tests its own methods reveals a general lesson: the replication unit has to match the phenomenon's own claimed mechanism — Time-Series Momentum's real, decades-old edge is invisible on arbitrary calendar blocks (0 of 5) but clear on real crisis episodes (9 of 13). Applied to this program's own instrument selection, restricting standard TSMOM to only the instruments it can actually predict well (via Paper 11's measured predictability limit) beats the unfiltered version in 3 of 5 real historical periods, full-sample Sharpe 0.32 vs. 0.29 — the paper's central result. A narrower, secondary finding survives the same scrutiny on two individual instruments (JPM, XLB), once recomputed as a real point estimate rather than the industry-standard, beta-adjusted statistic that had inflated four other candidates.
...
**Predictability-informed momentum and decision-layer sophistication (Paper 16):**
- **Data:** yfinance daily adjusted close, a 12-instrument universe for the decision-layer search and TSMOM/XSMOM benchmarking, the full 22-instrument Paper 12 panel for the broader replication-unit and statistic-choice demonstrations
- **Method:** two threads. (1) Decision-layer sophistication tested directly at real scale — a PPO reinforcement-learning agent and a genetic algorithm (800 candidates × 1,500 generations) sizing positions from every predictive signal this program has produced, predictability-informed conviction weighting, and breadth expansion from 5 to 40 candidate targets — none produce a repeatable edge, a sixth lever added to Papers 12–15's wall. (2) A methodology demonstration: Time-Series Momentum and Cross-Sectional Momentum, both published and unfitted, tested against arbitrary calendar blocks versus real, independently identified crisis episodes, showing the replication unit alone can flip a verdict from a total loss to a 9-of-13 win; the same discipline applied to instrument selection (restricting TSMOM to instruments with a genuine predictability pocket near its own 252-day lookback) produces this paper's central result, and applied to the choice of statistic (real mean-difference versus Jensen's alpha) shows 12 of 22 instruments flip sign depending on which is used — the basis for never using Jensen's alpha to evaluate this program's own methods, at any sample size.
- Full pipeline documented in `notebooks/predictor_v1/` (scripts 82–89) and `notebooks/decision_layer_ceiling_paper_draft.md` (PDF also available, published: zenodo.org/records/21842311)
...
- **Decision-layer search, six real attempts, one answer (2026-08, formalized in Paper 16, now published: zenodo.org/records/21842311): large-scale automated policy search does not buy past the ceiling either, and the reason is consistent across methods.** A genetic-algorithm search (population 800 × generations 1,500 = 1.2M real backtested policies, all three predictive signal types — price forecasts, τ*/pocket structure, CPE probability) showed genuine training-period learning that overfit decisively out of sample (1/12 instruments positive, pooled alpha -10.86%/yr). Two further GA fixes — regime-diverse fitness (selecting for consistency across 4 real training sub-periods) and training extended back to 2005 to include the 2008 GFC — were tried directly: the first made the result *worse* (cross-validating within a window can't manufacture exposure to a regime absent from all of it), the second gave a small genuine improvement (2/12, -9.73%/yr) but still no edge. A structurally different fourth attempt (real PPO — GAE, a learned value function, genuinely path-dependent state a GA population can't use) converged to the same place by a different route: after fixing a leverage-gaming reward bug, the trained policy still settles into a near-constant, state-independent position for 6 of 12 instruments, with real signal-conditioned variation only for JPM, XOM, and EURUSD=X. Full methodology: `notebooks/predictor_v1/82_ga_decision_layer_search.py`, `83_ga_regime_diverse_fitness.py`, `84_ga_gfc_extended_training.py`, `85_ppo_decision_layer.py`.
- **A second, independent methodological finding from the same investigation: OLS/Jensen's-alpha is never used to evaluate this program's own methods, at any sample size — not a style preference, a demonstrated instability.** Computing TSMOM's own alpha two ways on identical return series — a real mean-difference versus Jensen's alpha (OLS regression intercept, beta-adjusted) — flips sign on 12 of 22 instruments in this program's own panel (JPM: -10.99% vs. +2.21%), with no relationship to how much exposure the strategy carried. The mechanism: least-squares is the maximum-likelihood estimator specifically under a Gaussian-error assumption, and squaring residuals lets a handful of extreme days (this program's entire subject matter) dominate the fit; its standard-error/t-statistic inference layer is separately an explicit large-*n* construct that structurally cannot certify a real, small effect regardless of true size. A larger sample only hides the second problem — it leaves the first, the estimator's own point estimate being distorted rather than informed by extreme events, fully intact. Full methodology: `notebooks/tsmom_stat_choice_check.py`.
- **Paper 16 published (2026-08): the six-instrument/Jensen's-alpha framing in the two entries above is superseded by the paper's final, correctly-scoped result.** Recomputing the six-instrument convergence claim above as a real mean-difference (per the OLS finding directly above) rather than the originally-cited Jensen's alpha changes the picture: only JPM and XLB hold up positive under every method and every statistic checked (JPM master-model +0.78%/yr vs. TSMOM's -10.99%/yr; XLB master-model +0.97%/yr vs. TSMOM's -9.64%/yr). GLD, XLE, XLU, and MSFT looked positive under Jensen's alpha but did not survive recomputation. Kelly-sized position sizing shows larger numbers for all six but runs meaningfully elevated market exposure (beta 1.3–2.0 across the six checked), so part of that edge is leverage, not timing skill. The paper's actual headline result is separate from this instrument-level finding: restricting standard TSMOM to only the instruments it can actually predict well (the predictability-limit-informed instrument selection described above) beats the unfiltered baseline in 3 of 5 real historical periods, Sharpe 0.32 vs. 0.29 — the paper is titled and framed around this broader result, not the narrower two-instrument finding. Full methodology: `notebooks/alpha_generated_plot.py` (recomputes Kelly-sized/master-model alpha as real mean differences), `notebooks/decision_layer_ceiling_paper_draft.md` (published: zenodo.org/records/21842311).
- **Block-count robustness check on Paper 16's central result (2026-08): the win-count framing is more fragile than the underlying edge.** Paper 16's headline "3 of 5 real historical blocks" was tested at only one specific block count. Re-running the identical pocket-filtered-vs-baseline comparison at 10 different block counts (3 to 20, mechanical equal-length splits of the same 2006–2026 history — no randomisation involved, block edges are a deterministic function of the count) found the win-count itself ranges from 40% to 67% across those splits, dipping below 50% at two of them (3 of 7, 4 of 10) — the paper's own 5-block number sits on the favorable end of that range, not the middle. The average annual excess return, in contrast, stayed positive and stable at every single block count tested (+0.41% to +0.46%/yr), with pocket-filtered TSMOM's mean return per block above the baseline's at all 10 splits. The mechanism: at the block counts where the win-count dips below 50%, the median individual block still shows pocket-filtered trailing, but a handful of blocks carry large enough wins to keep the average positive — the same crisis-concentration signature already documented for TSMOM itself in Section 4.1, showing up one level down in the filtered version too. Full methodology and a companion infographic: `notebooks/tsmom_block_count_sensitivity.py`, `notebooks/paper16_pitsmom_infographic.html`.
...
- [Predictability-Informed TSMOM Infographic (Paper 16)](https://quantarram.github.io/quant-regime-research/notebooks/paper16_pitsmom_infographic.html) — a longer-form explainer: schematics of the TSMOM engine and the one change Predictability-Informed TSMOM makes to it (same engine, a smaller pre-screened universe), plus a robustness check re-running the paper's block comparison at 10 different block counts (3 to 20) — the average-return edge holds at every one, even though the paper's own "3 of 5" win-count framing is more sensitive to that specific choice of 5.
...
- [Five Levers Against the Ceiling (Paper 15)](notebooks/predictor_v1/p15_graphical_abstract.png) — a graphical summary of every lever tested against the measured predictability limit across Papers 14 and 15 (architecture, depth, training-window size, loss function, calibrated uncertainty), and the one result that reshapes the headline: climatology's own unmodeled uncertainty beats a carefully calibrated synthetic alternative on more than half the panel.
- [Predictability Filter vs. Standard Momentum (Paper 16)](notebooks/paper16_graphical_abstract.png) — the paper's central result in one image: restricting Time-Series Momentum to only the instruments it can actually predict well wins 3 of 5 real historical blocks versus 2 of 5 unfiltered; a smaller panel shows the two instruments (JPM, XLB) where this program's own signals beat standard TSMOM under every real point estimate checked.

---

## Research Papers

- [Paper 16: Predictability-Informed TSMOM Beats the Standard Momentum Trading Strategy](https://zenodo.org/records/21842311)
