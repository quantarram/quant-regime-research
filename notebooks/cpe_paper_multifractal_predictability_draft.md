...
Theoretical predictability limits of the atmosphere have been derived, in prior work by the author and others, from a multifractal cascade formalism in which the correlated and decorrelated components of a turbulent field's polyspectrum are tracked as a function of lead time, with the predictability limit defined as the time at which correlated energy falls to a fixed fraction of total energy (Ramanathan et al., 2019; Ramanathan & Satyanarayana, 2019, 2021). This paper investigates whether the same formalism transfers to financial markets. We show that it does not transfer directly: financial return/price-increment fields are empirically conservative (Hurst-type non-conservation exponent H ≈ 0), which collapses the moment-scaling relationship (ξ(q) = qH − K(qη) + qK(η)) that the atmospheric derivation depends on, and removes the spatial dimension (and with it, the entire generalized-scale-invariance anisotropy apparatus — sphero-scale, aspect ratio, vertical stratification exponent) that the atmospheric formula's lead-time dependence is built on. We therefore abandon the parametric route and construct the correlated and decorrelated structure functions directly and empirically, defining the predictability limit operationally as the lag at which they are equal, and — going further — treating the full gap between them as a function of lag as the object of interest rather than a single crossing time. Applied to a raw (untransformed) multi-asset universe spanning equities, commodities, crypto, rates, FX, and volatility, this reveals that the correlated fraction of the moment budget is bounded within an instrument-specific range across the entire 300-trading-day lag window tested — never reaching full correlation or full decorrelation, unlike the atmospheric picture's terminal state. But the *shape* of that boundedness is not uniform, and is not uniformly unlike the atmosphere either: classifying instruments by how many times their correlated and decorrelated components swap dominance reveals three distinct regimes. A minority of instruments (Treasury bonds and a few sector ETFs) remain correlation-dominant for the entire year tested, never crossing into decorrelation at all. One instrument (VIX) shows, at typical (q=2) moment order, a single clean transition that closely resembles the atmospheric formalism's own monotonic picture, just plateauing at a bound rather than reaching zero — though this resemblance disappears at higher moment order (q=4), where VIX's extreme fluctuations show the same repeated-crossing behavior as everything else. The majority of equities and Bitcoin show that repeated-crossing behavior throughout: predictability that comes and goes in discrete, recurring "pockets" at specific lags rather than decaying once and staying gone. A pocket at approximately 21–24 trading days recurs across most equities tested; a second pocket near 252 trading days (one calendar year) is prominent for SPY specifically, and coincides with the strongest validated-signal concentration independently found for SPY by the authors' unrelated Conditional Probability of Exceedance (CPE) framework — a nonparametric methodology sharing no assumptions with the structure-function approach used here. We argue this cross-method convergence, together with the observed asset-class and moment-order-dependent heterogeneity in crossing behavior, supports treating the pocket structure as a genuine, economically meaningful feature of market structure for the instruments that show it, rather than an artifact of either method or a universal law applying uniformly across all instruments.

## Plain Language Summary

Weather forecasters have a rigorous way of asking "how far into the future can this storm, in principle, ever be predicted?" — not limited by today's computers or models, but by the physics of turbulence itself. This paper asks the same question of financial markets, starting from the exact mathematical machinery the author previously used to answer it for the atmosphere. That machinery turns out not to transfer directly as a formula: it assumes a kind of field (like wind, which "forgets itself" as you look further ahead) that financial prices do not resemble once you actually measure their statistical properties — prices behave more like an already-settled, self-contained quantity than like a field that dissipates over time. Rather than forcing the atmospheric formula to fit, we built a direct, data-driven version of the same underlying idea — comparing how much of a market's statistical structure remains "linked to its own past" versus "genuinely new," at every possible time horizon out to about 14 months. The results are more varied than a simple "markets are nothing like weather" story: a handful of instruments (Treasury bonds, a few sector funds) stay just as predictable for the entire 14 months without ever losing that edge, one instrument (the VIX volatility index) shows a transition that looks remarkably like the weather picture for its typical day-to-day moves — just settling at a plateau instead of fading to nothing — though that resemblance disappears for its more extreme moves. Most stocks and Bitcoin, though, show something the atmosphere never does: predictability that comes and goes in sharp, recurring pockets rather than fading once and staying gone — one pocket around a month, and for the S&P 500 specifically, one around a year. That one-year pocket in the S&P 500 also shows up, independently, in the author's separate, differently-built market-signal framework (CPE), which is a meaningful cross-check that this is a real feature of markets rather than a coincidence of one particular method.
...
We define the predictability limit at moment order q as the lag τ at which C(τ,q) = D(τ,q) (equivalently, the correlated fraction C/(C+D) = μ = 0.5, matching the atmospheric convention), when such a crossing exists. Because — as Section 5.2 documents — the financial correlated/decorrelated relationship does not decay monotonically for most instrument/moment-order combinations tested (though it does for a minority), we generalize beyond a single crossing time and treat the *entire* gap G(τ,q) = C(τ,q) − D(τ,q) as the object of interest, ranking lags by local maxima of G(τ,q) rather than reporting a single Δt_p — a definition general enough to accommodate whichever of the three regimes (Section 5.2) a given instrument turns out to show.
...
### 5.2 Financial predictability is bounded — but its shape varies by instrument and moment order
...
This bounded-range property holds across the full fifteen-instrument sample, not just the nine shown above (Section 5.6, Figure 10, summarizes it across the complete sample). The market appears to maintain a permanent floor of both persistence and stochasticity: it never fully "forgets" its own recent state, and never becomes perfectly self-predictable either. This is consistent with — and arguably a structure-function-level restatement of — the well-documented finite, long-memory autocorrelation structure of financial volatility (Ghashghaie et al., 1996; Bacry, Muzy & Delour, 2001).

That much is true across every instrument. But "bounded" describes only where the correlated fraction ends up, not the *shape* of how it gets there — and on that question, the fifteen instruments do not tell one uniform story. To make this precise rather than impressionistic, we counted how many times C(τ) and D(τ) swap which one is larger, across the full 300-day window, and classified each instrument/moment-order combination into one of three regimes: **persistent** (zero crossings — one side dominates for the entire window), **single-crossing** (1–5 crossings — effectively one transition, the atmospheric-schematic shape), or **oscillating** (>5 crossings — repeated pocket structure, no single crossing time). **Figure 7** shows the result for all fifteen instruments at both q=2 and q=4.

![Figure 7](predictability_paper/figures/fig7_crossing_typology.png)
*Figure 7. Number of correlated/decorrelated crossings over the 300-day lag window, for all fifteen instruments at q=2 (larger markers) and q=4 (smaller markers), colored and shaped by regime: green squares (persistent, 0 crossings), gold diamonds (single-crossing, 1–5), red circles (oscillating, >5). Background shading marks the same three regions.*

At q=2 (typical-magnitude fluctuations), the sample splits three ways. Four instruments — IWM, XLE, XOM, and TLT — are **persistent**: their correlated fraction stays above 0.5 for the *entire* tested year, with zero crossings into decorrelation-dominance at all. This is not the atmospheric picture at all (the atmosphere always eventually decorrelates), but for the opposite reason from the pocket story below: rather than losing and regaining predictability, these instruments simply never lose it within the window tested. One instrument — ^VIX — is **single-crossing**: it starts correlation-dominant, crosses once into decorrelation-dominance around lag 10, and stays there for the rest of the range, with no further reversals. This genuinely is the atmospheric formalism's own shape (Figure 1), just plateauing at a bound instead of continuing to zero. (EUR/USD is nominally single-crossing too, but as Section 5.4 shows, its gap magnitude is numerically negligible throughout, so this is closer to a null result dressed up as a regime than a real single transition.) The remaining nine instruments — SPY, QQQ, XLK, XLF, AAPL, MSFT, JPM, GLD, and BTC-USD — are **oscillating**: 21 to 79 crossings apiece, the repeated pocket structure discussed in Section 5.3.

The regime is not fixed, however — it depends on moment order. At q=4 (extreme-magnitude fluctuations), all but one instrument becomes oscillating, including every member of the q=2 persistent group (IWM: 0→32 crossings; XLE: 0→81; XOM: 0→30; TLT: 0→17) and ^VIX itself (3→7 crossings, moving from single-crossing to oscillating). Only EUR/USD remains non-oscillating at q=4, and again for the negligible-magnitude reason noted above. In other words: an instrument's *typical* price moves can look persistently stable or atmosphere-like, but its *extreme* moves — the ones that actually matter for tail risk — show the same recurring-pocket structure almost universally. This mirrors, in a structural rather than a magnitude sense, the finding from the authors' own atmospheric work (Ramanathan & Satyanarayana, 2019) that higher-order moments capture qualitatively different predictability behavior than second-order statistics alone: there, higher q meant a *shorter* predictability limit for the same shape of decay; here, higher q means a *qualitatively different shape* of predictability altogether, not just a shorter one.

### 5.3 Predictability pockets

For the oscillating-regime instruments identified in Section 5.2 — most equities and Bitcoin at q=2, and nearly everything at q=4 — G(τ,q) shows local maxima recurring at specific lags rather than a single decay time (visible as the star markers in Figure 6, and directly as peaks in the fine-grained lag scan of **Figure 8**). A pocket at approximately 21–24 trading days recurs across nearly every equity tested: SPY (τ=22 is the top boundary-corrected tradeable q=4 pocket, gap=1142), QQQ (τ=22 is the single strongest q=4 pocket of the entire 300-day scan, gap=1623), IWM, XLK (τ=22, gap=6.99), XLF, XLE, AAPL (τ=22, gap=67.5), GLD (τ=22 present though weaker, gap=332), and TLT (τ=21, gap=0.54) — all at q=4, i.e., in their extreme-moment-order oscillating regime, since several of these (IWM, XLE, TLT) are only oscillating at q=4 to begin with, persistent at q=2. ^VIX also shows a pocket near lag 20 in its own q=4 curve (Figure 8's lower panel), consistent with its q=2→q=4 regime shift noted above.

![Figure 8](predictability_paper/figures/fig8_predictability_pockets.png)
*Figure 8. Correlated fraction (q=4) against lag for SPY (top) and VIX (bottom), full 1–300 trading-day range at daily resolution, with CPE's own horizon grid (21, 63, 126, 252 trading days) marked as vertical dashed lines. Local peaks are visible near the 21-day and 126-day lines for SPY, and near the 21-day line for VIX.*
...
The atmospheric predictability formalism this paper began from rests on three pillars: a spatial dimension providing an independent axis alongside lead time; a genuinely non-conservative (H > 0), growing-fluctuation field; and a monotonic, one-way correlated-to-decorrelated energy cascade terminating in total decorrelation (Figure 1). The first two pillars do not survive transposition to financial return series at all — there is no spatial axis (Section 3.1), and the natural raw field is empirically conservative (Section 3.2, Figure 4). The third is more subtle than a clean pass/fail: it survives, in a bounded, plateauing form, for exactly one instrument at typical moment order (^VIX at q=2, Section 5.2, Figure 7), fails in the opposite direction (predictability that never decays at all within the tested window) for four others (IWM, XLE, XOM, TLT, also at q=2), and fails in the way emphasized throughout this paper — bounded, oscillating, organized into pockets rather than a single decay time (Section 5.3) — for the majority of instruments, and for nearly all of them once extreme (q=4) rather than typical moves are considered (Section 5.2, Figure 7). What survives universally is not the atmospheric *shape*, but the *definition* of predictability loss as a crossing (or, more generally, a local balance) between correlated and decorrelated components of the moment budget — and, empirically, that definition still produces economically sensible, cross-validated results (Section 5.5, Figure 9) once applied directly rather than through the atmospheric closed-form solution, regardless of which of the three shapes a given instrument happens to take.
...
Applying the author's prior atmospheric multifractal predictability formalism directly to financial markets fails as a closed-form theory, for identifiable and physically meaningful reasons: financial return/price-increment fields are empirically conservative (Figure 4), and lack the spatial dimension the atmospheric derivation's lead-time dependence is constructed from (Section 3.1). Retaining only the formalism's core definition — predictability loss as the point where correlated and decorrelated components of the moment budget balance — and measuring both components directly and empirically (Figures 6, 7), rather than through the atmospheric closed-form solution, produces results that are bounded rather than reaching full correlation or decorrelation for every instrument (Figure 10), but whose *shape* is genuinely heterogeneous rather than uniformly non-atmospheric: a small minority of instruments remain correlation-dominant for the entire year tested with no crossing at all; one instrument (^VIX) shows a single clean transition matching the atmospheric formalism's own shape at typical moment order, though this resemblance vanishes for its extreme moves; and the majority of instruments — nearly all of them, once extreme rather than typical moves are considered — show the paper's central finding of discrete, recurring predictability pockets rather than a single decay time (Section 5.2, Figure 7; Section 5.3, Figure 8). This typology is internally consistent, robust to a boundary-detection bug once corrected (Section 5.1), and — for SPY's ~252-day pocket specifically — independently corroborated by a completely unrelated nonparametric methodology (CPE, Figure 9) developed for a different purpose. We take this as evidence that the pocket structure reported here, where it occurs, reflects genuine market phenomenology rather than an artifact of the method used to find it — and that the heterogeneity itself (which instruments show it, and at which moment order) is as much a finding as the pockets are.
