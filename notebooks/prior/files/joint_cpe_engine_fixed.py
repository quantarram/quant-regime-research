...
# ── EPISODE UTILS (shared canonical implementation, fixes Section 20.6 discrepancy)
...
for (y, tau_f, q_y, direction), group in groups:
    ...
    while len(selected) < MAX_PREDICTORS:
        ...
        if len(selected) >= 2:
            ...
            # Use episode_utils.compute_episode_conviction so this matches
            ...
            results.append({
                ...
                "episode_hit_rate":    None if (episode_hit_rate is None or np.isnan(episode_hit_rate)) else round(episode_hit_rate, 4),
