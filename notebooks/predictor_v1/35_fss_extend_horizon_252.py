"""
Add the 252-trading-day (~1yr) horizon to results_fss_true_unified.json,
per the user's point that CPE's own multifractal cascade work found the
longest horizons (~1yr) to be where predictability is strongest -- 32_*.py
only went out to 63d. Computes ONLY horizon=252 (identical methodology,
identical everything else to 32_fss_true_unified.py) and merges it into
the existing results file rather than recomputing 1/5/21/63d again.

Run: python 35_fss_extend_horizon_252.py
...
t0 = time.time()
print("=" * 60)
print(f"  FSS EXTEND: adding {NEW_HORIZON}d horizon to existing results")
print("=" * 60)
...
if NEW_HORIZON not in existing["horizons"]:
    existing["horizons"].append(NEW_HORIZON)
    existing["horizons"].sort()

n_added = 0
for tkr in ALL_TICKERS:
    res = run_instrument(tkr, NEW_HORIZON)
    if res is None:
        print(f"  {tkr} @ {NEW_HORIZON}d: skipped (insufficient data)  [{time.time()-t0:.0f}s]")
        continue
    existing_results.setdefault(tkr, {})
    existing_results[tkr][str(NEW_HORIZON)] = res
    n_added += 1
    avgs = {v: grid_avg(res, v) for v in VARIANT_EXTRA_COLS}
    print(f"  {tkr} @ {NEW_HORIZON}d: n={res['n_oos']['both']}, clim={avgs['climatology']:.4f}, "
...
print(f"\nAdded {NEW_HORIZON}d horizon for {n_added} instruments, merged into {results_path}")
