"""
One single-panel plot per instrument, identical style for all 25 (SPY
included) -- same axes, same color mapping, same everything, per the user's
explicit correction that the earlier plots were inconsistent (different
model sets/colors for SPY vs. the rest, four panels per instrument instead
of one). Consumes results_fss_unified.json (30_fss_unified.py).

Run: python 31_fss_unified_plots.py
Output: fss_plots_unified/<TICKER>.png
"""
import json
import os
import matplotlib
...
PLOT_DIR = os.path.join(OUT_DIR, "fss_plots_unified")
...
data = json.load(open(os.path.join(OUT_DIR, "results_fss_unified.json")))
